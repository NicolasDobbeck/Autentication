...
//Funcao para retornar apenas o registro baseado no ID
const selectAuthByPassword = async function (email) {
    ...
    //Criamos um objeto do tipo RecordSet (rsFuncionarios) para receber os dados do BD
    //através do script SQL (select)
    const rsFuncionario = await prisma.$queryRaw `select cast(id as float) as id, email, senha from tbl_funcionario where email = ${email}` ;
