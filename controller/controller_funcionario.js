...
const jwt = require("../authentication/jwt_funcionario")
...
const autenticarFuncionario = async function (email, senha){
    ...
    const dadosFuncionario = await funcionario.selectAuthByPassword(email);

    if (dadosFuncionario) {
        if (dadosFuncionario[0].senha == senha) {
...
const atualizarFuncionario = async function (funcionario) {
    ...
    {
        //import da model de funcionario
        const atualizarFuncionario = require('../model/model_funcionario.js');
